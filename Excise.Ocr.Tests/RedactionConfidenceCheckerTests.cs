...
/// <summary>
/// #650: a per-document runtime check comparing excise's own extraction
...
public class RedactionConfidenceCheckerTests
{
    ...
    [Fact]
    public void ClassifyPage_ExciseFindsNothing_OracleFindsRealText_IsSevere()
    {
        // The #637 shape: excise extracts (near-)nothing on a page that
        // genuinely has substantial text, per an independent oracle.
        var result = RedactionConfidenceChecker.ClassifyPage(47, "",
            "This entire page of real content was invisible to excise's own extraction " +
    ...
    public void ClassifyPage_PartialExtractionGap_IsDegradedNotSevere()
    {
        ...
        // excise finds most of it but is missing a meaningful chunk — degraded, not severe.
        var excise = "Alpha bravo charlie delta echo foxtrot golf hotel india";

        var result = RedactionConfidenceChecker.ClassifyPage(1, excise, oracle);
    ...
    {
        var result = RedactionConfidenceChecker.ClassifyPage(1, "some excise text", null);
    ...
    [Fact]
    public void ClassifyPage_OracleTextTooShortToBeSignal_IsOkRegardlessOfExciseText()
    {
