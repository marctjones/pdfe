...
/// measures how much pdfe's own extraction agrees with an independent
...
{
    /// <summary>
    /// <paramref name="pdfeText"/>'s letter/digit count as a fraction of
    ...
    /// </summary>
    public static double CoverageRatio(string pdfeText, string oracleText)
    {
        var oracleChars = CountLetterOrDigit(oracleText);
        if (oracleChars == 0) return 1.0;
        return (double)CountLetterOrDigit(pdfeText) / oracleChars;
    }
    ...
    /// about).
    /// </summary>
    public static double BigramJaccard(string pdfeText, string oracleText)
    {
        var a = Normalize(pdfeText);
