...
public class PdfSearchableConverterTests
{
    ...
    // independent tools (mutool, ghostscript), not pdfe's own extractor —
    ...
    public void MakeSearchable_ThenRedactText_RemovesBothTheInvisibleLayerAndTheRasterInk()
    {
        ...
        // proves: pdfe's image redaction (ImageRedactor.ShouldRemoveImageDo)
        ...
        var scanPath = Path.Combine(Path.GetTempPath(), $"pdfe-searchable-scan-{Guid.NewGuid():N}.pdf");
        ...
        try
        {
            ...
            var madeSearchablePath = Path.Combine(Path.GetTempPath(), $"pdfe-searchable-out-{Guid.NewGuid():N}.pdf");
            doc.Save(madeSearchablePath);

            try
            {
                // Step 3: verify with an INDEPENDENT extractor (mutool), not
                // pdfe reading its own page.Text.
                ...
                mutoolText!.ToUpperInvariant().Should().Contain(secret,
                    "the invisible layer must make the word findable by a tool that is not pdfe");
                ...
                var redactedPath = Path.Combine(Path.GetTempPath(), $"pdfe-searchable-redacted-{Guid.NewGuid():N}.pdf");
    ...
    private static Pdfe.Core.Document.PdfRectangle BoundsOf(PdfPage page, string word)
    {
        ...
        return new Pdfe.Core.Document.PdfRectangle(
    ...
    private static int FindWordStart(System.Collections.Generic.IReadOnlyList<Pdfe.Core.Text.Letter> letters, string word)
    {
        var text = string.Concat(letters.Select(l => l.Value));
        return text.IndexOf(word, StringComparison.OrdinalIgnoreCase);
    }

    private static double InkFractionIn(SKBitmap bmp, Pdfe.Core.Document.PdfRectangle box, double pageHeight)
    {
