...
/// "pdfe now draws something" is not sufficient evidence the fix is real;
/// an independent renderer (mutool) must agree ink appears where pdfe
...
public sealed class ShadingPatternMeshDifferentialTests
{
    ...
    public void Type5LatticeMeshPatternFill_MutoolAgreesInkIsDrawn()
    {
        ...
        try
        {
            ...
            var pdfeInk = NonWhiteFraction(pdfe);
            var mutoolInk = NonWhiteFraction(mutool!);

            pdfeInk.Should().BeGreaterThan(0.5,
            ...
            mutoolInk.Should().BeGreaterThan(0.5,
                "mutool (independent of pdfe) should also see the lattice mesh cover the page, " +
                "confirming the fixture itself is valid and not just something pdfe hallucinates");
        }
        finally
        {
            File.Delete(pdfPath);
    ...
    public void Type7TensorPatchMeshPatternFill_MutoolAgreesInkIsDrawn()
    {
        ...
        try
        {
            ...
            pdfeInk.Should().BeGreaterThan(0.2,
            ...
            mutoolInk.Should().BeGreaterThan(0.2,
                "mutool (independent of pdfe) should also see the tensor patch cover a substantial " +
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"pdfe-shading-{Guid.NewGuid():N}.pdf");
