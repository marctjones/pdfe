...
public class MetadataScrubTests
{
    ...
    {
        // Custom Info-dict keys (e.g. /pdfeAppVersion) should be left alone —
        // we only scrub the standard PDF spec keys.
        var pdf = BuildPdfWithMetadata(title: "T", customInfoKey: "pdfeAppVersion", customInfoValue: "1.0");
        using var doc = PdfDocument.Open(pdf);

        doc.ScrubMetadata();

        doc.Info.Should().NotBeNull();
        doc.Info!.GetStringOrNull("pdfeAppVersion").Should().Be("1.0");
    }
