...
{
    /// <summary>Oracle agrees closely with pdfe's own extraction (or nothing to compare).</summary>
    Ok,
...
/// <summary>
/// Compares pdfe's own text extraction against an independent oracle
...
public sealed class RedactionConfidenceChecker
{
    ...
    /// available oracle. Must be called before any page is mutated — pdfe's
    ...
    public RedactionConfidenceReport CheckDocument(PdfDocument document, string? sourceFilePath = null)
    {
        ...
        // pdfe's own extraction reads through the shared PdfDocument/page
        ...
        var pdfeTexts = new string[pageCount];
        for (int i = 0; i < pageCount; i++)
        {
            try { pdfeTexts[i] = document.GetPage(i + 1).Text ?? ""; }
            catch { pdfeTexts[i] = ""; }
        }
        ...
        try
        {
            if (useMutool)
            {
                ...
                {
                    tempPdfPath = Path.Combine(Path.GetTempPath(), $"pdfe-confidence-{Guid.NewGuid():N}.pdf");
        ...
        for (int i = 0; i < pageCount; i++)
            pages.Add(ClassifyPage(i + 1, pdfeTexts[i], oracleTexts[i]));
    ...
    /// <summary>
    /// Pure classification: given pdfe's own extraction and an oracle's for
    ...
    /// </summary>
    public static RedactionConfidencePageResult ClassifyPage(int pageNumber, string pdfeText, string? oracleText)
    {
        if (oracleText == null)
        ...
        var coverage = TextSimilarity.CoverageRatio(pdfeText, oracleText);
        var similarity = TextSimilarity.BigramJaccard(pdfeText, oracleText);
