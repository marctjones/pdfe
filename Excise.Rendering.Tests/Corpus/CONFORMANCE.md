...
The conformance validation harness tests excise against industry-standard PDF test corpora to verify:
...
SKIP_LARGE_CORPUS=0 dotnet test Excise.Rendering.Tests \
...
3. If readable externally, file a bug against excise: `Issue: ConformanceTest failed for <filename>`
...
2. **Actual veraPDF Tool Integration**
   ...
   - Compare against excise's results
   - Identify false positives (excise parses, veraPDF validates)
...
timeout 30 dotnet test Excise.Rendering.Tests \
...
This is expected! excise is a parser, not a validator. The test suite verifies:
...
- `Excise.Rendering.Tests/Corpus/SmokeCorpusTests.cs` — Visual rendering tests
- `Excise.Rendering.Tests/Corpus/EncodingDifferencesTests.cs` — Character encoding tests
