...
/// <c>Pdfe.Ocr.Tests/PdfSearchableConverterTests.cs</c> and, with real
/// tesseract, by <c>PdfEditor.Tests/Integration/MakeSearchableWiringTests.cs</c>.
/// </summary>
...
public class MakeSearchableDialogUiTests
{
    ...
    private static Button? FindButtonByName(Avalonia.LogicalTree.ILogical root, string automationName) =>
        CollectControls(root)
            .OfType<Button>()
            .FirstOrDefault(button => AutomationProperties.GetName(button) == automationName);

    private static TextBlock? FindTextByAutomationOrContent(Avalonia.LogicalTree.ILogical root, string text) =>
        CollectControls(root)
            .OfType<TextBlock>()
            .FirstOrDefault(tb => string.Equals(tb.Text, text, StringComparison.Ordinal));

    private static System.Collections.Generic.IEnumerable<Control> CollectControls(Avalonia.LogicalTree.ILogical root)
    {
        var stack = new System.Collections.Generic.Stack<Avalonia.LogicalTree.ILogical>();
        stack.Push(root);
        while (stack.Count > 0)
        {
            var node = stack.Pop();
