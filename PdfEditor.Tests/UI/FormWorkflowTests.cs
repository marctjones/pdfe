...
public class FormWorkflowTests
{
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"pdfe-form-workflow-{Guid.NewGuid():N}.pdf");
    ...
    public async Task SaveFileAsAsync_PreservesFilledInteractiveFormValue()
    {
        ...
        var outputPath = Path.Combine(Path.GetTempPath(), $"pdfe-form-filled-{Guid.NewGuid():N}.pdf");
        try
        {
    ...
    {
        var inputPath = WriteTempFormPdf();
        var outputPath = Path.Combine(Path.GetTempPath(), $"pdfe-form-flat-{Guid.NewGuid():N}.pdf");
        try
        {
            var vm = new MainWindowViewModel();
            await vm.LoadDocumentAsync(inputPath);
